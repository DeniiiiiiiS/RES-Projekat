...
mycursor = DB.cursor()
# kreiranje tabele tabledata3 ako ne postoji
mycursor.execute("create table tabledata3 (id int, dataset int, "
...
def insert_process(id_data, dataset, code, value):
    if code == "CODE_DIGITAL":
        print("Code is CODE_DIGITAL, inserting data into table tabledata3")
...
def check_deadband(id_data, dataset, code, value):
    mycursor.execute("select value from tabledata3 where code = %s", code)
...
def insert(id_data, dataset, code, value):
    mycursor.execute("insert into tabledata3(id, dataset, code, value, date), VALUES (%s, %s, %s, %s, now())",
