...
mycursor = DataBaseSet1.cursor()
# kreiranje tabele DataSet1
mycursor.execute("CREATE TABLE DataSet1 (ID int PRIMARY KEY, "
...
def insert_process(id_data, code, value):
    if code == "CODE_DIGITAL":
        print("Code is CODE_DIGITAL, inserting data into table DataSet1")
...
def check_deadband(id_data, code, value):
    mycursor.execute("SELECT VALUE from DataSet1")
...
def insert(id_data, code, value):
    mycursor.execute("INSERT INTO DataSet1(ID, CODE, VALUE, DATE), VALUES (%s, %s, %s, %s)",
