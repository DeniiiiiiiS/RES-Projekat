...
def create_database():
    ...
    if myresult[0] == "Yes":
        ...
        print("Reader2: Creating database database_reader")
...
def mydb_connection(host_name, user_name, user_password):
    ...
    try:
        ...
        print("Reader2: Connection to MySQL database_reader successful")
    except Error as e:
...
def read_values_by_code(connection, code_number):
    ...
    if not myresult:
        ...
        print("Reader2: ID |///| DATASET |///| CODE          |///| VALUE |///| DATE")
        for x in myresult:
            print(f"Reader2: {x[0]}        {x[1]}             {x[2]}         {x[3]}           {x[4]}")
...
def insert_process(connection, id_data, dataset, code_number, value):
    ...
    if code == "CODE_DIGITAL":
        print(f"Reader2: Code is '{code}', inserting data into table tabledata2")
        return insert(connection, id_data, dataset, code, value)
    else:
        print("Reader2: Code isn't CODE_DIGITAL, checking deadband...")
...
def check_deadband(connection, id_data, dataset, code_number, value):
    ...
    for row in myresult:
        if (abs(row[0] - value) / ((row[0] + value) / 2)) * 100 > 2:
            i += 1
    if i >= 1:
        print("Reader2: Difference between values is greater than 2%, inserting data into table tabledata2")
        return insert(connection, id_data, dataset, f"'{code}'", value)

    else:
        print("Reader2: No insertion, difference between values is less than 2%")
